                ...
                if (items.length != 0) {
                  ...
                  for(let i=0;i<items.length;i++)
                  {
                    let request3 = await db
                      .collection("items")
                      .where(firestore.FieldPath.documentId(), "==", items[i].itemId)
                      .get();
                  ...
                  arrItem.forEach((obj) => {
                    let cantidad = items.find(
                      (objItem) => objItem.id === objItem.id
                    ).cantidad as number;
                    arrNotaDePedido.push({ cantidad, itemId: obj });
                  });
                 
                  dispatch(NotaDePedido_ReduxAction_AddArray(arrNotaDePedido));
                }
